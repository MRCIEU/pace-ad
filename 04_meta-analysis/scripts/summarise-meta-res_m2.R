...
plots_outfile <- "results/meta-summary/man-qqs_m2.tiff"
summary_outfile <- "results/meta-summary/comparison-summary_m2.RData"
qq_outfile <- "results/meta-summary/meta-qqs_m2.png"

meta_files <- "results/metal-res/m2a.txt results/metal-res/m2b.txt results/metal-res/m2c.txt"
meta_files <- unlist(str_split(meta_files, " "))

m2_man_outfile <- "results/meta-summary/meta-m2a-manhattan.png"
m2_man_outfile <- "results/meta-summary/meta-m2b-manhattan.png"
m3_man_outfile <- "results/meta-summary/meta-m2c-manhattan.png"

n<-gsub("results/metal-res/","",meta_files)
n<-gsub("m2a","Unadjusted",n)
n<-gsub("m2b","Maternal factors adj",n)
n<-gsub("m2c","Main",n)
n<-gsub(".txt","",n)
nlab<-n
...
models<-nlab
m2a_model <- which(models%in%c("Unadjusted"))
m2b_model <- grep("Maternal", models, value = T)
m2c_model <- grep("Main", models, value = T)
names(qq_plots)<-nlab
theme_set(theme_bw(base_size = 12))   # or theme_minimal(), theme_cowplot()
all_plots <- cowplot::plot_grid(qq_plots[[m2a_model]], mans[[m2a_model]], 
                                qq_plots[[m2b_model]], mans[[m2b_model]],
