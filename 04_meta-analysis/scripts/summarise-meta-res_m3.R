...
plots_outfile <- "results/meta-summary/man-qqs_m3.tiff"
summary_outfile <- "results/meta-summary/comparison-summary_m3.RData"
qq_outfile <- "results/meta-summary/meta-qqs_m3.png"

meta_files <- "results/metal-res/m3a.txt results/metal-res/m3b.txt results/metal-res/m3c.txt"
meta_files <- unlist(str_split(meta_files, " "))

m1_man_outfile <- "results/meta-summary/meta-m3a-manhattan.png"
m2_man_outfile <- "results/meta-summary/meta-m3b-manhattan.png"
m3_man_outfile <- "results/meta-summary/meta-m3c-manhattan.png"

n<-gsub("results/metal-res/","",meta_files)
n<-gsub("m3a","Unadjusted",n)
n<-gsub("m3b","Maternal factors adj",n)
n<-gsub("m3c","Main",n)
n<-gsub(".txt","",n)
nlab<-n
...
models<-nlab
m3a_model <- which(models%in%c("Unadjusted"))
m3b_model <- grep("Maternal", models, value = T)
m3c_model <- grep("Main", models, value = T)
names(qq_plots)<-nlab
theme_set(theme_bw(base_size = 12))   # or theme_minimal(), theme_cowplot()
all_plots <- cowplot::plot_grid(qq_plots[[m3a_model]], mans[[m3a_model]], 
                                qq_plots[[m3b_model]], mans[[m3b_model]],
