...
plots_outfile <- "results/meta-summary/man-qqs-fig1.tiff"
summary_outfile <- "results/meta-summary/comparison-summary_fig1.RData"
qq_outfile <- "results/meta-summary/meta-qqs_fig1.png"

meta_files <- "results/metal-res/m1c.txt results/metal-res/m2c.txt results/metal-res/m3c.txt"
meta_files <- unlist(str_split(meta_files, " "))

m1_man_outfile <- "results/meta-summary/meta-m1-manhattan_fig1.png"
m2_man_outfile <- "results/meta-summary/meta-m2-manhattan_fig1.png"
m3_man_outfile <- "results/meta-summary/meta-m3-manhattan_fig1.png"

n<-gsub("results/metal-res/","",meta_files)
n<-gsub("m1c","childhood AD",n)
n<-gsub("m2c","early-onset AD",n)
n<-gsub("m3c","persistent AD",n)
n<-gsub(".txt","",n)
nlab<-n
...
models<-nlab
m1c_model <- which(models%in%c("childhood AD"))
m2c_model <- grep("early-onset AD", models, value = T)
m3c_model <- grep("persistent AD", models, value = T)
names(qq_plots)<-nlab
theme_set(theme_bw(base_size = 12))   # or theme_minimal(), theme_cowplot()
all_plots <- cowplot::plot_grid(qq_plots[[m1c_model]], mans[[m1c_model]], 
                                qq_plots[[m2c_model]], mans[[m2c_model]],
                                qq_plots[[m3c_model]], mans[[m3c_model]],
