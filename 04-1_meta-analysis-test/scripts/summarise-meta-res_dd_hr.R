...
m1_man_outfile <- "results/meta-summary/meta-dd-manhattan.png"
m2_man_outfile <- "results/meta-summary/meta-sr-manhattan.png"
...
models<-nlab
dd_model <- which(models%in%c("Doctors Diagnosis"))
sr_model <- grep("Doctors Diagnosis & Rash", models, value = T)
hr_model <- grep("Hanifin-Rajka", models, value = T)
names(qq_plots)<-nlab
...
all_plots <- cowplot::plot_grid(qq_plots[[dd_model]], mans[[dd_model]], 
                                ...
                                qq_plots[[hr_model]], mans[[hr_model]],
